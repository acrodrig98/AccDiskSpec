...
class FUOri:
    ...
    # Initializer / Instance Attributes
    def __init__(self, r_star, r_inner, r_outer, m_star, m_dot, inc, a_v, pl_index, atm_table):
        self.r_star = r_star
        self.r_inner = r_inner
        self.r_outer= r_outer
        self.m_star = m_star
    ...
    def calculateFracLums(self, wave_lower, wave_upper, data_type, x_axis_type):
        ...
        for i in range(len(self.annuli)):
            ...
            temps_diff[i] = tempFUOriDisk([annulus.r_inner], self.r_star, self.m_dot,\
                      self.m_star, self.pl_index) \
            - tempFUOriDisk([annulus.r_outer], self.r_star, self.m_dot, self.m_star, self.pl_index)
            
        # Inner 
        inner_disk_diff = np.abs(self.annuli[1].temp - self.annuli[0].temp)
        temps_diff[0] = inner_disk_diff*2
