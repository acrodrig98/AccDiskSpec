...
class Annulus:
    ...
    def setSpectrumFromDirec(self, spec_model, template_temp, template_grav, m_star):
        if spec_model == 'stellar atmospheres':
            grav_test = 4.0
            try:
                r_avg = np.mean((self.r_inner, self.r_outer))
                if np.log10(G*m_star/(r_avg**2)) > 2.0:
                    grav_test = 4.0
                else:
                    grav_test = 1.5
                self.wavelengths, self.lums = getLumFromDirec(self.temp, grav_test, self.directory, spec_model)
                print('Used stellar atmosphere at T = ' + str(int(self.temp)) + '.'\
                      + ' with log g = ' + str(grav_test))
            except:
    ...
    # Broadening        
    def broadenSpectrumByInst(self, inst_width):
        from astropy.convolution import Trapezoid1DKernel
        waves = self.wavelengths_interp
        lums = self.lums_interp
        lums_broad = convolve(lums, Trapezoid1DKernel(inst_width))
        self.lums_interp = lums_broad
...
class FUOri:
    ...
    # Initializer / Instance Attributes
    def __init__(self, r_star, r_inner, r_outer, m_star, m_dot, inc, a_v):
        self.r_star = r_star
        self.r_inner = r_inner
        self.r_outer= r_outer
        self.m_star = m_star
    ...
    def createAnnuliFromDirec(self, grav, directory, model):
        for i in range(len(self.temps)):
            ...
            annulus.setSpectrumFromDirec(model, self.temps[0], grav, self.m_star)
    ...
    def prepareAnnuli(self, wave_lower, wave_upper, binning, broaden, interp_type):
        ...
        for i in range(len(self.annuli)):
            ...
            # Broaden
            if broaden:
#                self.annuli[i].broadenSpectrumByInst(inst_width)
...
class Star:
    ...
    # Initializer / Instance Attributes
    def __init__(self, temp, grav, m_star, r_star, directory, inc, a_v):
        self.temp = temp
        self.grav = grav
        self.m_star = m_star
        self.r_star = r_star
